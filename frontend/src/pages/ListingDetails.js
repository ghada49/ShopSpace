...
export default function ListingDetails() {
  ...
  const insights = [
    { label: 'Foot Traffic', value: listing.foot_traffic ? `~${listing.foot_traffic}` : 'N/A', sub: 'Daily average', subColor: 'text-green-500' },
    { label: 'Exposure Score', value: listing.exposure_score ? `${listing.exposure_score}/10` : 'N/A', sub: 'Visibility rating', subColor: 'text-slate-500', valueColor: 'text-primary' },
    { label: 'Category', value: listing.category ? listing.category.charAt(0).toUpperCase() + listing.category.slice(1) : 'General', sub: 'Space type', subColor: 'text-slate-500' },
    { label: 'Size', value: listing.size || 'N/A', sub: 'Available space', subColor: 'text-slate-500' },
  ];
      ...
      <main className="max-w-7xl mx-auto w-full px-4 py-6 md:grid md:grid-cols-12 md:gap-8">
        ...
        <div className="md:col-span-8 space-y-8">
          ...
          <section className="space-y-4">
            ...
            <div className="flex gap-3 flex-wrap">
              {[
                ...
                listing.size && { icon: 'square_foot', label: listing.size },
          ...
          <section className="space-y-4">
            ...
            <div className="h-48 w-full rounded-xl bg-slate-100 flex items-center justify-center border border-slate-200">
              <div className="flex flex-col items-center gap-2 text-slate-500">
                <span className="material-symbols-outlined text-primary text-4xl">location_on</span>
                <span className="font-medium">{listing.location}</span>
                {listing.address && <span className="text-sm text-slate-400">{listing.address}</span>}
              </div>
            </div>
          </section>
        </div>
        ...
        <aside className="md:col-span-4 mt-8 md:mt-0">
          <div className="sticky top-24 p-6 bg-white border border-slate-200 rounded-2xl shadow-xl space-y-6">
            <div className="flex items-end gap-1">
              ...
              <span className="text-slate-500 pb-1">/ day</span>
            </div>
            ...
            <div className="space-y-3 border-t border-slate-100 pt-4">
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">${listing.price_per_day?.toLocaleString()} x {days} days</span>
